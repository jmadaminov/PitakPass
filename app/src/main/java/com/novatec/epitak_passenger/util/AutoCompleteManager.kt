...
import com.novatec.epitak_passenger.ui.interfaces.IOnPlaceSearchQueryListener
...
@ExperimentalSplittiesApi class AutoCompleteManager(builder: Builder) {
    ...
    private fun setupFromInputAutocomplete() {
        ...
        fromFeed = Autocomplete.on<PlaceFeedItemView>(from)
            ...
            .with(fromPresenter)
            .with(context!!.getDrawable(R.drawable.selector_rounded_corners_stroke))
            .with(3f)
            .build()
    ...
    fun setupListeners() {
        from!!.setOnEditorActionListener { _, actionId, _ ->
            when (actionId) {
                EditorInfo.IME_ACTION_DONE -> {
                    if (fromPresenter.getAdr() !=null && fromPresenter.getAdr()!!.itemCount > 0) {
        ...
        to!!.setOnEditorActionListener { _, actionId, _ ->
            when (actionId) {
                EditorInfo.IME_ACTION_DONE -> {
                    to!!.clearFocus()
                    if (toPresenter.getAdr() !=null && toPresenter.getAdr()!!.itemCount > 0) {
